...
{
public:
    virtual void AttachObserver(uint16_t addr, MemoryByteObserver* observer)
    {
    ...
    {
        typedef std::unordered_multimap<uint16_t, MemoryByteObserver*>::iterator iter;
    ...
    virtual bool NotifyObservers(uint16_t addr, uint8_t value)
    {
        ...
        for (iter it = range.first; it != range.second; ++it)
        {
            if (it->second)
            {
                it->second->UpdateMemoryAddr(addr, value);
                notified = true;
            }
        }
...
protected:
    std::unordered_multimap<uint16_t, MemoryByteObserver*> memoryObservers;
};